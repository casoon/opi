...
| Rust | `cargo fmt --check`, `cargo clippy` and `cargo rustdoc` with warnings denied, `cargo test --all-features` |

A tool applies when the package declares it as a dependency; its binary is taken from a
`node_modules/.bin` at or above the package. Where several tools answer for the same concern,
only the first detected one runs, and its name is shown next to the result.
